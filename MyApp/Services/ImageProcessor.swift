...
enum ImageProcessor {
    ...
    static func applyMask(original: UIImage, mask: UIImage) -> UIImage? {
        guard let ciOriginal = CIImage(image: original),
              let ciMask = CIImage(image: mask) else { return nil }

        let alphaMask = ciMask
            .applyingFilter("CIColorInvert")
            .applyingFilter("CIMaskToAlpha")

        let composited = ciOriginal
            .applyingFilter("CIBlendWithAlphaMask", parameters: [
                "inputMaskImage": alphaMask
            ])

        let context = CIContext()
        guard let output = context.createCGImage(composited, from: ciOriginal.extent) else { return nil }
