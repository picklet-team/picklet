            ...
            GeometryReader { geo in
                ...
                VStack(spacing: spacing) {
                    ...
                    ScrollViewReader { proxy in
                        ScrollView {
                            LazyVGrid(columns: Array(repeating: .init(.fixed(cellSize), spacing: spacing), count: columnsCount), spacing: spacing) {
                                ForEach(assets, id: \.localIdentifier) { asset in
                                    PhotoThumbnailCell(asset: asset, size: cellSize, manager: imageManager) { image in
                                        onImagePicked(image); dismiss()
                                    }
                                    .id(asset.localIdentifier)
                                    .background(
                                        GeometryReader { gp in
                                            Color.clear.preference(key: CellTopPreferenceKey.self, value: [asset.localIdentifier: gp.frame(in: .named("gridSpace")).minY])
                                        }
                                    )
                                }
                            }
...
struct PhotoThumbnailCell: View {
    ...
    private func loadThumb() {
        let opts = PHImageRequestOptions(); opts.deliveryMode = .highQualityFormat; opts.resizeMode = .exact
        let target = CGSize(width: size*UIScreen.main.scale, height: size*UIScreen.main.scale)
        manager.requestImage(for: asset, targetSize: target, contentMode: .aspectFill, options: opts) { img,_ in thumbnail = img }
    }

    private func requestFull() {
        let opts = PHImageRequestOptions(); opts.deliveryMode = .highQualityFormat; opts.resizeMode = .exact; opts.isNetworkAccessAllowed = true
        let target = CGSize(width: asset.pixelWidth, height: asset.pixelHeight)
        manager.requestImage(for: asset, targetSize: target, contentMode: .aspectFit, options: opts) { img,_ in if let img = img { onSelect(img) } }
    }
}
